...
$following_string = $server_Obj->get_user_following($id);

$following_array = explode(":", $following_string);

$shift = array_shift($following_array);

if (in_array($id, $following_array)) {

    if (($key = array_search($other_user, $following_array)) !== false) {
...
<div class="container w-100 mt-1">
    ...
    $num_row = count($following_array);

    if ($num_row == 0 or $following_string == 'n_value') {
